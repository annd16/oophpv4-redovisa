...
//     header("Location: \Anax\View\url($mount.'/index-session-in')");
//     exit;
// }
//
...
<body class="<?= $data['class'] ?>">
    ...
    <main class= <?= $class ?>>
        ...
        <title>Guess my number - <?= strtoupper($class2) ?> </title>
            ...
            <h1 class="h1-<?= $class ?>">A guessing game with <?= strtoupper($class2) ?> </h1>

            <navbar class=navbar>
                <a href="<?= \Anax\View\url($mount) ?>">Return to start page</a>
                <a href="../gissa/session">Reset game</a>
                <a href="<?= \Anax\View\url($mount."/index-session-in") ?>.php?destroy=true">Kill session</a>
...
$form = new Form($class, $done, $else, $guessedNumber);

$form->displayForm("session", "post");

// Test 180920
$formCheat = new Form($class, $done, $else);

$formCheat->displayForm("session", "post", true);
