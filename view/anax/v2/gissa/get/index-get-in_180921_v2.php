    ...
    <main class= <?= $data["method"] ?>>
        ...
        <title>Guess my number - <?= strtoupper($data["method"]) ?> </title>
            ...
            <navbar class=navbar>
                ...
                <a href="../gissa/<?= $class2 ?>">Reset game</a>
            </navbar>

<?php

// // Detta stycke kan nog flyttas upp
//
// $done = "false";
//
// if (!isset($_GET['theNumber'])) {
...
try {
    if (isset($_GET["cheat"])) {
        ...
        $result2 = $guess->checkTheGuess(htmlentities($_GET['guessedNumber']));
        ...
        if ($guess->getNoGuessesLeft() >= 0) {
            ...
            if (isset($_GET['guessedNumber'])) {
                ...
                <!-- <div class="result" id="result">
                <?= "<h4>The guessed number </h4>" . "<h3 class='$lastWord'>" . htmlentities($_GET['guessedNumber']) . "</h3>" . "<h4>  is  </h4>" . "<h3  class='$lastWord'>" . $result2 . "</h3>"
                . "<h4 class='guessesleft'>Number of guesses left: " . $guess->getNoGuessesLeft() ?>
                </div> -->

                <?php

                $result =  "<h4>The guessed number </h4>" . "<h3 class='$lastWord'>" . htmlentities($_GET['guessedNumber']) . "</h3>" . "<h4>  is  </h4>" . "<h3  class='$lastWord'>" . $result2 . "</h3>"
                . "<h4 class='guessesleft'>Number of guesses left: " . $guess->getNoGuessesLeft();
            }
        } else {
            // $exception = False;
...
$form = new Form($class2, $done, $else, $guessedNumber, $guess->getTheNumber(),  $guess->getNoGuessesLeft()-1);

$form->displayForm("get", "get");

// Test 180920
$formCheat = new Form($class2, $done, $else, null, $guess->getTheNumber(), $guess->getNoGuessesLeft());
